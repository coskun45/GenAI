...
class PandasGraphImporter(GraphImporterInterface):
    ...
    Notes
    -----
    Graph attributes are stored as additional edge data.
    If there is no id, one is created equal to the row number (count start at 1)
    Node and edge labels are supported
    ...
    @staticmethod
    def get_or_create_node(node_name, node_names):
    ...
    def _import(self, graph) -> Tuple[List[Dict], List[Dict], bool]:
        ...
        for i, row in graph.iterrows():
            edge_id = row.get('id', None)
            if edge_id is None:
                edge_id = i + 1

            _prop = {}
            _edge = {'id': edge_id, 'properties': _prop}

            source_id = self.get_or_create_node(row.get('source', None), node_names)
            target_id = self.get_or_create_node(row.get('target', None), node_names)

            if row.get('source', None) is not None and row.get('target', None) is not None:
                _edge = {'id': i, 'start': source_id, 'end': target_id, 'properties': _prop}
                for attr in graph.columns:
                    if attr not in ['source', 'target', 'id']:
                        if row.get(attr, None) is not None:
                            _prop[attr] = row[attr]

            _edge['properties'] = _prop
            edges.append(_edge)

        for node in node_names:
            _node = {'id': node, 'properties': {'label': node}}
            nodes.append(_node)
